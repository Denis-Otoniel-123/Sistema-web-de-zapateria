...
//	private ArrayList<Boleta_Detalle> detBoleta = new ArrayList<Boleta_Detalle>();
	...
	public Pnl_Content_Venta() {
		...
		lblValorPrecio = new JLabel("Q/. 00.00");
	...
	void limpiarSeccionCalzado(){
		lblValorPrecio.setText("Q/. 00.00");
		...
		txtUnidades.setText("0");
		txtTotal.setText("Q/. 00.00");
	}
	
	void mostrarDatosCalzado(CalzadoReporte obj){
		lblValorPrecio.setText(String.format("Q/. %.2f", obj.getPrecio_venta()));
		...
		txtTotal.setText(String.format("Q/. %.2f", obj.getPrecio_venta()));
	}
	...
	void generarTotal(){ // lee las unidades, calcula el total y lo imprime
		...
		int unid = leerUnidades();
		if(obj != null){
			txtTotal.setText(String.format("Q/. %.2f", obj.getPrecio_venta()*unid));
		}
...
//						modelo.removeRow(fila);
					...
					txtTotalVenta.setText(String.format("Q/. %.2f", getTotalVenta())); // si el numero de filas es 0, getToralVenta() siempre regresar� 0.0
				}
			}
		}
	...
	public void mousePressed(MouseEvent e) {
		if(e.getSource() == tblVenta){ // << 
			...
			txtTotalVenta.setText(String.format("Q/. %.2f", getTotalVenta()));
